...
from models import (
    ...
    create_transaction, delete_transaction, convert_estimate_to_invoice,
)
...
class QuotesView(ctk.CTkFrame):
    ...
    def build_ui(self):
        header = ctk.CTkLabel(self, text="Quotations / Estimates",
        ...
        build_toolbar(
            ...
            self.search_var,
            "Search estimates...",
            add_text="+ New Estimate" if self.user.has_permission("Quotes_Create") else None,
            add_command=self.show_builder if self.user.has_permission("Quotes_Create") else None,
            pady=(0, 5),
        )
    ...
    def refresh_list(self):
        ...
        estimates = get_all_transactions(self.search_var.get(), tx_type="Estimate")
        can_delete = self.user.has_permission("Quotes_Delete")
        can_export = self.user.has_permission("Export_PDF")

        if not estimates:
            ctk.CTkLabel(self.list_frame, text="No estimates yet.",
        ...
        for est in estimates:
            ...
            ctk.CTkLabel(info, text=f"EST-{est['id']:04d}",
                         font=ctk.CTkFont(size=13, weight="bold")).pack(anchor="w")
            ctk.CTkLabel(info, text=f"{est.get('client_name', 'Walk-in')}  |  {est['date']}",
            ...
            ctk.CTkButton(btn_row, text="View", width=50, height=24,
                          ...
                          command=lambda e=eid: self.view_estimate(e)).pack(side="left", padx=2)
            if can_export:
                ctk.CTkButton(btn_row, text="PDF", width=50, height=24,
                              font=ctk.CTkFont(size=11), fg_color="#8e44ad",
                              command=lambda e=eid: self.export_pdf(e)).pack(side="left", padx=2)
            ctk.CTkButton(btn_row, text="Invoice", width=55, height=24,
                          font=ctk.CTkFont(size=11), fg_color="#e67e22",
                          command=lambda e=eid: self.to_invoice(e)).pack(side="left", padx=2)
            if can_delete:
                ctk.CTkButton(btn_row, text="Del", width=40, height=24,
                              font=ctk.CTkFont(size=11), fg_color="#e74c3c", hover_color="#c0392b",
                              command=lambda e=eid: self.del_estimate(e)).pack(side="left", padx=2)

    def _show_empty_detail(self):
        for w in self.detail_frame.winfo_children():
            w.destroy()
        ctk.CTkLabel(self.detail_frame, text="Select an estimate or create a new one",
    ...
    def show_builder(self):
        ...
        ctk.CTkLabel(builder, text="New Estimate",
        ...
        ctk.CTkButton(btn_frame, text="Save Estimate", fg_color="#27ae60",
                      hover_color="#219a52", command=self.save_estimate).pack(side="left", padx=(0, 5))
    ...
    def _refresh_line_items(self):
        ...
        for idx, item in enumerate(self.line_items):
            ...
            if item["item_type"] == "service":
                maint_row = ctk.CTkFrame(self.items_display, fg_color="transparent")
                maint_row.pack(fill="x", padx=24, pady=(0, 2))
                ctk.CTkLabel(maint_row, text="Maintenance Schedule:",
                             font=ctk.CTkFont(size=10), text_color="gray").pack(side="left")
                maint_entry = ctk.CTkEntry(maint_row, width=200, height=24,
                                           placeholder_text="e.g. Monthly, Quarterly...")
                maint_entry.insert(0, item.get("maintenance_schedule", ""))
                maint_entry.pack(side="left", padx=5)
                maint_entry.bind("<FocusOut>",
                                 lambda e, ii=i, me=maint_entry: self._update_maint(ii, me))
    ...
    def _update_maint(self, idx, entry):
        self.line_items[idx]["maintenance_schedule"] = entry.get()
    ...
    def save_estimate(self):
        ...
        tx_id = create_transaction(
            ...
            tx_type="Estimate", status="Pending", notes=notes, items=self.line_items,
        )

        messagebox.showinfo("Success", f"Estimate EST-{tx_id:04d} saved!")
        self.refresh_list()
        self._show_empty_detail()

    # ── View / Actions ────────────────────────────────────────

    def view_estimate(self, est_id):
        for w in self.detail_frame.winfo_children():
            w.destroy()
        ...
        ctk.CTkLabel(detail, text=f"Estimate #{tx['id']:04d}",
                     font=ctk.CTkFont(size=18, weight="bold")).pack(anchor="w", padx=10, pady=(10, 2))
        ctk.CTkLabel(detail, text=f"Client: {tx.get('client_name', 'Walk-in')}  |  Date: {tx['date']}",
        ...
        if tx.get("notes"):
            ctk.CTkLabel(detail, text=f"Notes: {tx['notes']}", font=ctk.CTkFont(size=11),
        ...
        if self.user.has_permission("Export_PDF"):
            ctk.CTkButton(btn_frame, text="Export PDF", fg_color="#8e44ad",
                          command=lambda: self.export_pdf(est_id)).pack(side="left", padx=(0, 5))
        ctk.CTkButton(btn_frame, text="Convert to Invoice", fg_color="#e67e22",
                      command=lambda: self.to_invoice(est_id)).pack(side="left", padx=5)
        ctk.CTkButton(btn_frame, text="Back", fg_color="gray40",
                      command=self._show_empty_detail).pack(side="left", padx=5)

    def export_pdf(self, est_id):
        tx = get_transaction(est_id)
        items = get_transaction_items(est_id)
        if tx and items:
    ...
    def to_invoice(self, est_id):
        if messagebox.askyesno("Convert", "Convert this estimate to an invoice?\nThis will decrement stock for products."):
            convert_estimate_to_invoice(est_id)
            messagebox.showinfo("Done", "Estimate converted to Invoice.")
            self.refresh_list()
            self._show_empty_detail()

    def del_estimate(self, est_id):
        if messagebox.askyesno("Confirm", "Delete this estimate?"):
