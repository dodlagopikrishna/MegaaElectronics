...
def bootstrap_admin():
    ...
    cursor.execute(
        "INSERT INTO users (username, password_hash, role_id, is_active, must_change_password) VALUES (?, ?, ?, 1, 0)",
        (DEFAULT_ADMIN_USERNAME, default_hash, admin_role["id"]),
    )
...
def create_user(username, password, role_ids):
    conn = get_connection()
    cursor = conn.cursor()
...
def update_user(user_id, username, role_ids, is_active):
    conn = get_connection()
    cursor = conn.cursor()
    ...
    if is_default_admin(user_id):
        ...
        cursor.execute(
            ...
            (username, primary_role, is_active, user_id),
        )
